...
function checkPassword() {
    ...
    if ((passWord.value.trim() == "") || (passWord.value.trim().length < 8)) {
        ...
        let speclCharCount = 0;     //to count the number of specl char.s in the input 
        let spclChars = ['!', '@', '#', '$', '%', '&', '*'];    //spcl. char.s array                              
        if (regExPassword.test(passWord.value)) {                                         // password test
            if ((passWord.value == confirmPassword.value) && (regExPassword.test(confirmPassword.value))) {               //confirm password test
